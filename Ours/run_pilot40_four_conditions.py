...
from Ours.pilot40_protocol import (  # noqa: E402
    PROMPTS,
    PROMPT_TASK,
    PROTOCOL_VERSION,
    append_record,
...
def validate_features(
    ...
    region_count = int(region.shape[0])
    if region_count <= 0:
        raise ValueError(f"{slide_id}: region count R must be greater than zero")

    expected_patch_shape = (region_count, 48, 512)
    ...
    expected_cell_shape = (region_count, 48, 384)
    ...
    expected_index_shape = (region_count, 48)
    if (
    ...
    if patch_indices.dtype not in _INDEX_DTYPES or cell_indices.dtype not in _INDEX_DTYPES:
        raise TypeError(
            f"{slide_id}: patch/cell indices must use integer dtypes, got "
            f"{patch_indices.dtype} and {cell_indices.dtype}"
        )
    if patch_indices.dtype != cell_indices.dtype:
        raise TypeError(
            f"{slide_id}: patch/cell index dtypes must match, got "
            f"{patch_indices.dtype} and {cell_indices.dtype}"
        )
    if not torch.equal(patch_indices, cell_indices):
        raise ValueError(f"{slide_id}: patch/cell indices are not exactly equal")
    if torch.any(patch_indices < 0) or torch.any(patch_indices >= 256):
        raise ValueError(f"{slide_id}: patch indices must be in [0,255]")
    if any(torch.unique(row).numel() != 48 for row in patch_indices):
        raise ValueError(f"{slide_id}: each region must contain 48 unique patch indices")
...
def load_features(
    ...
    wsi_payload = _load_tensor_payload(
        feature_root / "wsi" / f"{slide_id}.pt", slide_id, "wsi"
    )
    region_payload = _load_tensor_payload(
        feature_root / "region_4k" / f"{slide_id}.pt", slide_id, "region"
    )
    patch_payload = _load_tensor_payload(
        feature_root / "patches_filtered" / f"{slide_id}.pt", slide_id, "patch"
    )
    cell_payload = _load_tensor_payload(
        cell_root / slide_id / "encoded_cell_features.pt", slide_id, "cell"
    )
...
def _is_current_success(
    ...
    if record.get("protocol_version") != expected["protocol_version"]:
        return False
    if record.get("task") != expected["task"]:
        return False
    ...
    if not isinstance(raw_answer, str):
        return False
    parsed_class = parse_answer(raw_answer, expected["task"])
    if record.get("parsed_class") != parsed_class:
        return False
    if type(record.get("correct")) is not bool:
        return False
    if expected["task"] == "binary":
        truth = "Renal" if expected["true_class"] in {"KIRC", "KIRP"} else "Breast"
    else:
        truth = expected["true_class"]
    return record["correct"] == (parsed_class == truth)
...
def _write_artifacts(
    ...
    summary = build_summary(successful)
    _write_json_atomic(output_dir / "summary.json", summary)
    _write_json_atomic(output_dir / "metrics.json", summary)
    return successful


def _write_preflight_errors(output_dir: Path, errors: list[dict[str, str]]) -> None:
    _write_json_atomic(output_dir / "preflight_errors.json", errors)
...
def run(
    ...
    for row in selection:
        try:
            ...
            print(
                f"Preflight failed for {row['slide_id']}: "
                f"{type(exc).__name__}: {exc}"
            )

    class_counts = Counter(row["true_class"] for row in selection)
    print(
        f"Total slides: {len(selection)}; class counts: "
        f"KIRC={class_counts['KIRC']}, KIRP={class_counts['KIRP']}, "
        f"IDC={class_counts['IDC']}, ILC={class_counts['ILC']}"
    )
    _write_preflight_errors(args.output_dir, preflight_errors)
    if preflight_errors:
        return 1
        ...
        for row in selected:
            ...
            for prompt_id, prompt in pending_prompts:
                ...
                try:
                    ...
                    parsed_class = parse_answer(raw_answer, task)
                    truth = (
                        "Renal"
                        if task == "binary" and row["true_class"] in {"KIRC", "KIRP"}
                        else "Breast"
                        if task == "binary"
                        else row["true_class"]
                    )
                    ...
                    print(
                        f"{row['slide_id']} {prompt_id}: "
                        f"{parsed_class} | {raw_answer}"
                    )
                except Exception as exc:
                    append_record(
                        jsonl_path,
    ...
    successful = _write_artifacts(args.output_dir, jsonl_path, expected_metadata)
    successful_keys = {
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Pilot-40 binary/four-class, open/fixed frozen MLLM experiment."
    )
