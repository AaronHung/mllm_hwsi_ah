...
\newcommand{\method}{UPD-R}               % Utility-weighted Policy Distillation + Replay
\def\x{{\mathbf x}}
...
counterfactual diagnostic gain, and we mitigate forgetting with
utility-weighted policy distillation over compressed navigation states.
Across two four-task and two-task TCGA sequences, our method recovers
\xx{XX}\% of the forgetting gap to joint training while storing only
kilobyte-scale states, and behavior-level metrics expose forgetting that
...
\textbf{(3) Mitigation.} We propose utility-weighted policy distillation with
compressed state replay (\method{}): old-task decision states are stored as
kilobyte feature tuples, and the new policy is regularized toward the old one
preferentially on high-utility states.
...
while the budget--forgetting interaction confirms redundancy masking;
\method{} restores behavioral stability (Jaccard $\xx{XX}$, action-KL
$\xx{XX}$) and improves final average accuracy over sequential fine-tuning and
standard CL baselines \cite{kirkpatrick2017ewc,li2017lwf}.
...
\cite{anderson2018r2r}, with topological-map agents such as DUET
\cite{chen2022duet} and history-aware transformers \cite{chen2021hamt};
recurrent hard attention dates to glimpse networks \cite{mnih2014ram}.
Recent work brings navigation to WSI \cite{pathagent2026,pathnavigate2026}.
These assume human demonstration paths or geometric success signals,
and train for a \emph{single} task; continual policy learning is untouched,
and TCGA offers no expert trajectories --- motivating our counterfactual
teacher and our focus on policy retention.
...
+ \lambda\, w(s)\,
\mathrm{KL}\big(\pi_\theta \| \pi_{\mathrm{old}}\big) \Big],
\label{eq:cl}
\end{equation}
with $w(s) = u(s)/\bar u$ clipped at $5$: replayed teacher targets keep the
policy competent on old states, and the distillation term anchors it to the
old policy \emph{most strongly where evidence choice mattered most}. States
whose gain landscape is flat consume almost no preservation capacity.
...
\cite{li2017lwf}, replay-only, uniform distillation, \method{}, and joint
training (upper bound). All share the identical frozen-evaluator protocol.
...
($\xx{XX}$ at $K{=}1$). \method{} restores behavioral stability and yields the
best AA / forgetting among CL methods at every budget, approaching joint
training with only $\xx{512}$ stored states per task.
...
Distill (unif.)  & \xx{--} & \xx{--} & \xx{--} & \xx{--} & \xx{--} & \xx{--} \\
\method{} (ours) & \xx{--} & \xx{--} & \xx{--} & \xx{--} & \xx{--} & \xx{--} \\
\midrule
Joint (upper)    & \xx{--} & \xx{--} & \xx{--} & \xx{--} & \xx{--} & \xx{--} \\
\bottomrule
\end{tabular}}
\end{table}
...
weighting}: replacing $w(s)$ with uniform weights degrades AA by $\xx{XX}$
and Jaccard by $\xx{XX}$, confirming that protecting high-stakes decisions is
what matters, not replay volume. (iii) \emph{Memory}: performance saturates
at $\xx{512}$ states/task ($<\!\xx{1}$\,MB); $\lambda$ trades stability for
plasticity smoothly (all in supplement).
...
trajectories, and utility-weighted distillation over compressed states
preserves acquisition behavior at kilobyte cost. Limitations: feature-space
