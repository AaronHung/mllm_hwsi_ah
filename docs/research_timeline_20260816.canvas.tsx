...
const phases: Phase[] = [
  ...
  {
    id: "p7",
    title: "階段 7 — Method Gate 跑完 + verdict 計算（8/16）",
    status: "completed",
    what:
      "Mac MPS 上的 33 單位 method gate（M1/M2/M3 三個主 config × 3 seeds × K∈{1,2,4}，加兩個 mini-arm 診斷單位）全部跑完。寫了一份 scripts/method_gate_verdict.py，把 v0.33.3 pre-registration 裡凍結的 g1–g4 判準逐字套用到跑完的結果上（跟凍結的舊對照組資料、已通過 checksum 的 MPS util_regen 數字比對），沒有『看到數字後才決定門檻』。",
    why:
      "Gate 存在的目的就是要在看到結果之前把『贏的標準』講清楚，這樣不管結果是好是壞都站得住腳，不會被質疑事後合理化。",
    result:
      "三個候選（M1/M2/M3）全部 GATE FAIL。M2（eq_pres）最接近：g2（utility 指標變好）與 g3（不輸 replay）都乾淨通過，但 g1（K=1 遺忘打平）與 g4（brca 最後任務的學習力）沒過。額外做了一個『單一 seed 敏感度』診斷：18 個 FAIL 子項裡有 15 個是『3 個 seed 裡只有 1 個把平均值拖過門檻』，不是 3/3 一致的訊號——這不會反轉 pre-registered 的判定，但是很重要的解讀線索（3 個 seed、每任務 15 筆測試資料，統計 power 本來就薄）。另外發現凍結的 ours_uniform 對照組資料從來沒有在 K=4 跑過，K=4 的比較誠實標成 N/A，沒有補跑新資料去填洞。",
    paper:
